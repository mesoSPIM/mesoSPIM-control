## Release candidate August 2022 [1.8.0]
:sparkles: Installation and updating from PyPi: `pip install mesospim-control`. Control software can be launched in Anaconda prompt from any folder, by either `mesospim-control` or `mesospim` command. Updating to a newer version viw `pip install upgrade mesospim-control`.

:sparkles: Icon for the desktop shortcut.

:gem: BigTIFF file support

:gem: Required space is calculated before acquisition: if the disk has insufficient free space, acquisition does not start (with error message).

:gem: Optional webcam window

:gem: Optional Image Contrast window in Alignment Tab (for calibration purposes)
...
:wrench: Log messages printed in `Log` tab for quick review, in addition to log file.
...
:wrench: Galvo and ETL of non-active arm are held still to minimize unnecesessary heating and stress.
...
:bug: fix: laser lines were hard-coded `0:7`, irrespective of config file settings.

:bug: fix: PI stage did not send position updates properly due to missing signal/slot connection: stages were frequently stuck outside of allowed range.

:bug: fix: Image and metadata files are closed properly if the acquisition is aborted.
