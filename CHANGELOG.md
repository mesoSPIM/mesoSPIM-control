...
- Writer settings come in two shared files: `config/plugins/writers/production_writers.py` (`MP_OME_Zarr_Writer` ring buffer 512, for a workstation with fast IO) and `config/plugins/writers/demo_writers.py`, which includes the former and only shrinks the ring buffer to 16 for a demo/simulation machine.
- Laser settings are split by the DAQ that drives the enable lines: `config/hardware/lasers/benchtop_PXI6733_lasers.py` (everything on the single PXI-6733 named `PXI1Slot4`, 11 of the example configs), `V5_PXI6733_lasers.py` (V5/V6: lasers on `PXI6733`, shutters on `PXI6259`, 13 configs), `cDAQ_lasers.py` (CompactDAQ, `cDAQ1Mod2`), and `demo_lasers.py`, which includes the benchtop file and only switches `laser`/`shutter` to `'Demo'`. Each DAQ file names the laser file it pairs with.
