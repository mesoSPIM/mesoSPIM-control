...
- The `plugins` dict (`path_list`, `first_image_writer`) moved into the shared `config/plugins/writers/default_writers.py`, so a user config only mentions it when its plugin locations or preferred writer differ from the default.
