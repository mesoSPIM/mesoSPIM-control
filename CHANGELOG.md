## Unreleased [1.26.0]
### New Features ✨
- 💎 **Two-level configuration files**: hardware definitions now live once in shared files under `mesoSPIM/config/hardware/` (`cameras/`, `DAQ/`, `stages/`, `lasers/`, `filterwheels/`, `objectives/`, `galvos/`, `ETLs/`), with writer settings in `mesoSPIM/config/plugins/writers/` and interface defaults in `mesoSPIM/config/UI/`. A user config file picks its hardware with `include('hardware/cameras/...')` and overrides only what is personal to it; anything assigned after the `include()` wins, and dicts of the same name (e.g. `startup`) merge key-by-key. `demo_config.py` is the reference two-level file. Existing single-file configs keep working unchanged — the new `config_format = 2` marker is optional and absent means legacy.
- Converter for existing config files: `python -m mesoSPIM.src.utils.convert_config my_old_config.py -o my_config.py` picks the best-fitting shared hardware file per category, writes the rest as overrides, drops settings the software no longer reads (including connection settings such as a COM port or servo id that the configured driver ignores), and verifies the result by loading both files and comparing them setting by setting (`--check` reports without writing). All 26 example configs convert with every old setting reproduced exactly. The single-file examples moved to `mesoSPIM/config/examples/format1(legacy)/`.
- Zoom/objective settings are split into shared files by the changer they drive: `config/hardware/objectives/Dynamixel_Olympus_MVX10.py` (Olympus MVX-ZB10 zoom body on a Dynamixel servo, 0.63x .. 6.3x, 15 rig configs), `Mitutoyo_revolver.py` (motorised 5-position revolver, positions `'A'` .. `'E'`, 9 configs), `benchtop_manual_objectives.py` (benchtop, objectives changed by hand, 12 configs) and `demo_zoom.py`. Each carries its `zoomdict`, its `pixelsize` table and the camera those pixel sizes were measured with.
- Config files are at most two levels deep: only the user file may call `include()`, and an included file that calls it raises an error naming the rule. Files that differ in a couple of settings (e.g. `demo_lasers.py` vs `benchtop_PXI6733_lasers.py`) repeat their content rather than chaining, so a setting is never more than one file away from the config you opened.
- `config/demo_config_format1(legacy).py`: the demo microscope written as a single legacy file, next to the two-level `demo_config.py`. The two load into exactly the same settings (checked by a test), so the formats can be compared side by side. Both are free of settings the software never reads: the demo filter wheel and zoom files lost the COM port, baud rate and servo id that their `'Demo'` drivers never open.
- The converter shortens float values that carry more digits than anyone typed to 5 significant digits (`'7.5x': 0.5666666666666667` becomes `0.56667`, `'galvo_l_phase': 0.4487989505128276` becomes `0.4488`) and reports each one. A value that already fits in 8 significant digits is left alone, so a deliberate `199.195` keeps its precision.
- The `plugins` dict (`path_list`, `first_image_writer`) moved into the shared writer settings under `config/plugins/writers/`, so a user config only mentions it when its plugin locations or preferred writer differ from the default.
- Writer settings come in two shared files: `config/plugins/writers/production_writers.py` (`MP_OME_Zarr_Writer` ring buffer 512, for a workstation with fast IO) and `config/plugins/writers/demo_writers.py`, the same file with the ring buffer shrunk to 16 for a demo/simulation machine.
- Laser settings are split by the DAQ that drives the enable lines: `config/hardware/lasers/benchtop_PXI6733_lasers.py` (everything on the single PXI-6733 named `PXI1Slot4`, 11 of the example configs), `V5_PXI6733_lasers.py` (V5/V6: lasers on `PXI6733`, shutters on `PXI6259`, 13 configs), `cDAQ_lasers.py` (CompactDAQ, `cDAQ1Mod2`), and `demo_lasers.py`, the benchtop file with `laser`/`shutter` switched to `'Demo'`. Each DAQ file names the laser file it pairs with.
- `config/hardware/DAQ/NI_benchtop_PXI1Slot4.py` renamed to `NI_benchtop_PXI6733.py` after the card it describes, matching `NI_benchtop_PXIe6738.py` (`PXI1Slot4` is only the NI MAX name of that card).
- Removed the duplicated binning setting: the binning in use has always been `startup['camera_binning']` (GUI, metadata, acquisition), while `camera_parameters['binning']` only seeded the initial value and was overwritten at startup. All five code paths now read `startup['camera_binning']`; leftover `camera_parameters['binning']` entries in existing config files are ignored and can be deleted.
- `startup['camera_line_interval']` is now optional and only needed for Hamamatsu cameras; other camera configs may omit it (the state default 75 µs is used). Removed the unused `startup['camera_sensor_mode']` state parameter and the dead `set_camera_sensor_mode()` code path — the Hamamatsu sensor mode is set from `camera_parameters['sensor_mode']`, as before. Leftover `camera_sensor_mode` entries in existing config files are ignored.
- Parameters tab → "Save parameters to config": values that live in an included hardware file are now appended to the user's config as a `startup.update({...})` override block instead of being silently skipped, and values inside commented-out lines are no longer rewritten.
...
### Bugfixes 🐛
- Any driver name containing "Demo" now selects the simulated device, for the camera, the waveform generation, the stage, the filter wheel and the zoom (the laser enabler already worked this way). `'Demo'` is what the demo config files write from now on; the older spellings `'DemoCamera'`, `'DemoWaveFormGeneration'`, `'DemoStage'`, `'DemoZoom'` keep working unchanged. This also fixes `'DemoFilterWheel'`, which 4 config files in the repository set and which used to abort startup with "Filterwheel type not supported".
- The config file check at startup now validates the zoom settings. `zoomdict` and `pixelsize` must have the same keys and `startup['zoom']` must be one of them, otherwise mesoSPIM refuses to start: previously a zoom position without a pixel size passed startup and raised a `KeyError` inside the image writer, i.e. in the middle of an acquisition. It also warns (without stopping) when `startup['pixelsize']` disagrees with `pixelsize[startup['zoom']]` - the state now starts from the table value in any case -, when a pixel size differs by more than 5% from the camera pixel pitch divided by the magnification in its label (a table left over from another camera), and when the camera pixels are not square, since only one pixel size per zoom position is recorded. Checked against all 59 config files in the repository: none fails, 24 warn.
- Converter: `startup['camera_line_interval']` is dropped for a non-Hamamatsu camera. Only the Hamamatsu class pushes it to the hardware; every other camera reads it into a variable nothing uses, and the value recorded in the metadata comes from the state default either way.
