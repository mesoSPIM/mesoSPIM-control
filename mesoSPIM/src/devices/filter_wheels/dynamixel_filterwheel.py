...
class DynamixelFilterWheel(QtCore.QObject):
    def __init__(self, filterdict, filterpositiondict, COMport, identifier=2, baudrate=1000000):
        super().__init__()
        from ..zoom.dynamixel import dynamixel_functions as dynamixel

        self.filterdict = filterdict
        self.filterpositiondict = filterpositiondict
    ...
    def set_filter(self, filter, wait_until_done=False):
        ...
        if filter in self.filterdict:
            self._move(self.filterpositiondict[self.filterdict[filter]], wait_until_done)
            self.filter = filter
        else:
